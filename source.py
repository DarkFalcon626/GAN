...
def weights_init(m):
    '''
    Initializes the model m weights randomly from a normal distribution.
...
class Generator(nn.Module):
    ...
    def train(self, disc, data_fake, loss, optimizer, device):
        '''
        Trains the generator updating the models parameters based on
        the optimizer function to minimized the loss function to learn to 
        create better data to trick the discriminator disc.
        ...
        data_fake : Tensor
            The fake data generated by the generator function.
        loss : Torch loss function
            The loss fucntion to minimize.
        ...
        -------
        float
            The value of the loss functions.
        '''
        
        b_size = data_fake.shape[0] # Get the size of the batch.
        
        # Use the real labels on the fake data.
        real_label = torch.ones(b_size, 1, dtype=float).to(device)
        
        optimizer.zero_grad() # Zero the gradients of the optimizer.
        
        ## Feed the fake data through the discriminator and find the loss.
        output = disc.forward(data_fake)
        loss_val = loss(output, real_label)
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        x = self.main(x)
        x = torch.flatten(x, 1) # Flattens the convolation into a column vector.
        x = self.fc(x)
        
        return x
    
    def train(self, data_real, data_fake, loss, optimizer, device):
        '''
        Trains the discriminator updating the models parameters based on
        the optimizer function to minimized the loss function to learn to 
        distinguish between the real data data_real and the fake data 
        data_fake.
        ...
        self.train() # Set the model to train.
        
        ## Find the batch size.
        b_size = data_real.shape[0]
        
        ## Create the real and fake labels
        real_labels = torch.ones(b_size, 1, dtype=float).to(device)
        fake_labels = torch.zeros(b_size, 1, dtype=float).to(device)
        
        optimizer.zero_grad() # Zero the gradient of the optimizer.
        
        ## Feed the real data throught the model and determine the loss.
        output_real = self.forward(data_real).view(-1)
        loss_real = loss(output_real, real_labels)
        
        ## Feed the fake data throught the model and determine the loss.
        output_fake = self.forward(data_fake)
        loss_fake = loss(output_fake, fake_labels)
        
        ## Create the gradients from the real and fake losses.
        loss_real.backward()
        loss_fake.backward()
